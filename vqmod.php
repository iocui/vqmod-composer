...
abstract class VQMod
{
	 ...
	 * VQMod::bootup()
	 *
	 * @param string $path File path to use
	 * @param bool $logging Enable/disabled logging
	 * @return null
	 * @description Startup of VQMod
	 */
	public static function bootup($path = '', $logging = true)
	{
	...
	public static function modCheck($sourceFile, $modificationFile = false)
	{
		...
		if (sha1($fileData) != $fileHash) {
			...
			file_put_contents(self::path(self::$checkedCache, true), implode($lines), LOCK_EX | LOCK_NB); // 加锁防高并发写入失败 By PGCAO
	...
	{
		$xmlPath = dirname(__FILE__) . DIRECTORY_SEPARATOR . 'xml' . DIRECTORY_SEPARATOR; // 固定XML文档路径为当前文件同根下的xml目录 By PGCAO

		// $xmlPath = self::path('vqmod/xml', true); // 原方式

		self::$_modFileList = glob($xmlPath . '*.xml');
		...
		$xml_folder_time = filemtime($xmlPath);
		...
		if (self::$_devMode || !file_exists($modCache)) {
			...
			if (!empty($mods))
				self::$_mods = substr($mods, 0, 2) == 'a:' ? unserialize($mods) : false;
			if (self::$_mods !== false) {
				return;
			}
		}
...
class VQModObject
{
	...
	public function applyMod($mods, &$data)
	{
		...
		foreach ($mods as $mod) {
			...
			$tmp = (array)$this->_explodeData($tmp);
			$lineMax = count($tmp) - 1;
	 ...
	 *
	 * @param DOMDocument $node <modification> node to be parsed
	 * @return null
